...
import { Button, message, Drawer, DatePicker, Space } from "antd";
...
const FormAdminRent = () => {
  ...
  const btnCapNhat = async () => {
    try {
      const res = await adminUser.adminPutRentId(params.id, values);
      console.log("res: ", res);
    } catch (error) {
      console.log(error);
    }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="pickdate">
            <Space direction="vertical" size={12}>
              <RangePicker
                onChange={onChange}
                renderExtraFooter={() => "Chọn Ngày và Giờ"}
                showTime
